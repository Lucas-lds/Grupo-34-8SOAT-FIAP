...
resource "aws_instance" "db_client" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "mysql -h ${aws_db_instance.mysql_rds.address} -u ${var.mysql_user} -p${var.mysql_password} -e 'CREATE DATABASE IF NOT EXISTS ${var.mysql_database};'",
      "mysql -h ${aws_db_instance.mysql_rds.address} -u ${var.mysql_user} -p${var.mysql_password} ${var.mysql_database} < /home/ec2-user/init.sql" # Caminho do arquivo SQL na instância
    ]
  ...
  depends_on = [aws_db_instance.mysql_rds, aws_key_pair.generated_key]
}